...
export const OfficialsAccessGate: React.FC<Props> = ({ onAuthenticated }) => {
  ...
  return <section className="mx-auto max-w-md rounded-2xl border border-sky-200 bg-white p-6 shadow-sm">
    <div className="mb-5 flex items-center gap-3"><div className="rounded-xl bg-sky-100 p-3 text-sky-700"><KeyRound className="h-5 w-5"/></div><div><h2 className="text-base font-semibold text-stone-900"><TranslatedText>Officials Secure Login</TranslatedText></h2><p className="text-xs text-stone-500"><TranslatedText>Sign in with an official account stored in PostgreSQL.</TranslatedText></p></div></div>
    <form onSubmit={login} className="space-y-3">
      ...
      <button disabled={loading} className="w-full rounded-xl bg-sky-700 px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-50"><TranslatedText>{loading ? 'Signing in...' : 'Sign in to Officials Dashboard'}</TranslatedText></button>
    </form>
    <div className="mt-4 flex gap-2 text-[11px] text-stone-500"><ShieldCheck className="h-4 w-4 shrink-0 text-sky-600"/><TranslatedText>Admin can create sub-official accounts. Sub-officials can access monitoring but cannot create more officials.</TranslatedText></div>
  </section>;
};
