...
export const HomeView: React.FC<HomeViewProps> = ({ onNavigate }) => {
  return (
    <section className="space-y-6">
      <div className="relative overflow-hidden rounded-2xl border border-stone-800 bg-stone-900 px-6 py-8 sm:px-10 sm:py-12">
        <div className="absolute right-0 top-0 h-full w-1/3 bg-gradient-to-l from-emerald-950/50 to-transparent" />
        <div className="relative max-w-3xl">
          <div className="mb-4 flex items-center gap-2 text-xs font-mono uppercase tracking-[0.18em] text-emerald-400">
            <Activity className="h-4 w-4" />
            CareBridge
          </div>
          <h2 className="max-w-2xl text-3xl font-semibold tracking-tight text-stone-50 sm:text-5xl">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-stone-700 px-4 py-2.5 text-sm font-semibold text-stone-200 transition-colors hover:border-stone-500 hover:bg-stone-800"
            >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {destinations.map(({ section, title, description, icon: Icon, accent }) => (
          <button
            ...
            className="group rounded-xl border border-stone-800 bg-stone-900/80 p-5 text-left transition-colors hover:border-stone-600 hover:bg-stone-800"
          >
            <div className="flex items-start justify-between gap-4">
              <span className={`rounded-lg border p-2 ${accent}`}>
                <Icon className="h-5 w-5" />
              </span>
              <ArrowRight className="h-4 w-4 text-stone-600 transition-transform group-hover:translate-x-1 group-hover:text-stone-300" />
            </div>
            <h3 className="mt-5 text-sm font-semibold text-stone-100">{title}</h3>
            <p className="mt-2 text-xs leading-6 text-stone-400">{description}</p>
          </button>
        ))}
      </div>
